...
// constructor 
kt_algorithm::kt_algorithm(Framework* fram) : JetFinder(fram){ }

bool kt_algorithm::InitRun(){ return true; }
bool kt_algorithm::FinalizeRun(){ return true;}


bool kt_algorithm::EventLoop(){
    //getting input Clusters
    const DataCollection* inputs = fStorage->RetrieveData("CaloClusterCollection", TString("DICaloCluster") );
    assert(inputs);
    if(inputs->size() == 0) { return true; }                //if there is no Cluster there is nothig to be done
    ...
    while(Jets.size() != 0){
        ...
        * calculation of all beam and particle distances
        ...
        // getting the element with the smallest distance
        M::iterator itMin = min_element(distance.begin(), distance.end(), valueComparer);
...
void kt_algorithm::makeHisto(const DICaloJet* jet){
